...
const CampaignDetail: React.FC = () => {
  ...
  const fetchItems = useCallback(
    async (page: number, limit: number) => {
      if (!id || id === "new")
        return { entities: [], total: 0, offset: 0, limit };
  ...
  const fetchEntities = useCallback(
    async (page: number, limit: number = 10, search: string = "") => {
      let response: any;
      if (importType && importValue) {
        if (importType === "book") {
          response = await getBookItems({
            page,
            limit,
            search,
            bookId: importValue,
  ...
  const renderItem = useCallback(
    (item: DungeonMonster, selected: boolean, onSelect: () => void) => (
      <ItemCard
  ...
  const handleSubmit = async (values: Dungeon) => {
    try {
      ...
      console.error(error);
      message.error(
        `Failed to ${id && id !== "new" ? "update" : "create"} campaign`
      );
    }
  };
  ...
  return (
    <PageLayout
      title={
        id && id !== "new" ? `Edit Campaign (id: ${id})` : "Create Campaign"
      }
    ...
    >
      <div className="campaign-progress">
        ...
        {id && id !== "new" && (
          <Button
            className="view-monsters-button"
            type="link"
            onClick={() => navigate(`/campaigns/${id}/monsters`)}
          >
            <h2>View Monsters</h2>
          </Button>
        )}
      ...
      <Card className="campaign-detail-card">
        <Form
          ...
          className="campaign-detail-content"
        >
          <TitleField />
          <MarkdownField
            name="description"
        ...
        {id && id !== "new" ? (
          ...
          <span className="items-not-available">
            - items can only be added to existing campaigns -
          </span>
        )}
      </Card>
    </PageLayout>
